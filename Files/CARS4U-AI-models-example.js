...
var g_area_model_deployment_id   = '';  // <-- Get this from the "Business Area" model deployment details page in Watson Studio
var g_action_model_deployment_id = '';  // <-- Get this from the "Business Action" model deployment details page
...
getAuthToken( g_username, g_password ).then( function( token )
{
    ...
    processInput( token,
                  ...
                  g_area_model_deployment_id ).then( function( area_result )
    {
        //printResult( area_result );

        g_sample_payload.fields.push( 'Business_Area' ); // <-- Enter 'Business_Ar' to see the error handling
        g_sample_payload.values[0].push( area_result.values[0][ area_result.values[0].length - 1 ] )

        processInput( token,
                      g_sample_payload,
                      g_action_model_deployment_id ).then( function( action_result )
        {
            printResult( action_result );

        } ).catch( function( action_error )
        {
            printError( "Action model:\n" + action_error );

        } );

    } ).catch( function( area_error )
    {
        printError( "Area model:\n" + area_error );
